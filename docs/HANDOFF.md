...
Last updated: 2026-06-11 (evening session)
...
Six commits, all verified (Windows `cargo check` clean, **83/83 tests** on both
WSL and Windows toolchains, `npm run build` clean). **Local only — not pushed.**
...
resize/anchoring per mode, archive Play button end-to-end, the focus guard, and
the timeout-transcribe path. First launch after these changes runs migration 002
and the defaults-v4 migration; both are tested but watch the logs
(`%APPDATA%\com.natkins.localdictate\logs`) on first real run.
...
2. **Notes feature** — designed but NOT approved in detail; confirm specifics
   ...
   REST + OAuth, not MCP**.
3. **Transcribe audio/video files** — owner wants this. Reuse the bundled
   whisper-cli (`resources/bin/windows/`); a "Transcribe file" view: pick file,
   pick model, save/copy the transcript. ffmpeg is NOT bundled — either require
   WAV/extract audio another way or document the dependency.
...
5. Carry-overs from `docs/STATUS_AND_NEXT_STEPS.md`: flip repo public,
...
- `app/src-tauri/src/audio.rs` — capture, silence auto-stop, timeout thread (freeze fix lives in `timeout_recording_for_app`)
